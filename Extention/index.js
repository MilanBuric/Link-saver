...
// This function checks the storage and then retrieves the leads from storage and renders them in the UI
chrome.storage.sync.get(["leads"], ({ leads }) => {
    // If leads exist in storage, assign them to myleads and render them
    if (leads) {
        myleads = leads;
        renderLeads();
    }
...
inputBtn.addEventListener("click", function() {
    ...
    if (inputValue) {
    // Check if the URL already exists in the leads list
    ...
    // If the input is a valid URL, fetch the page title and save it
    // If the input is not a valid URL, it will still be saved with the hostname
